...
export function DashboardButton({ className }: DashboardButtonProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      <button
        onClick={() => {
          console.log('🔗 Direct navigation to minimal dynamic test');
          window.location.href = '/t/zenzone/minimal';
        }}
        className="w-full bg-red-300 hover:bg-red-400 text-white font-semibold py-1 px-4 rounded text-center block text-xs"
      >
        🔴 Minimal Dynamic Test
      </button>
      
      <button
        onClick={() => {
          console.log('🔗 Direct navigation to zenzone direct (no dynamic routing)');
          window.location.href = '/zenzone-direct';
        }}
        className="w-full bg-blue-300 hover:bg-blue-400 text-white font-semibold py-1 px-4 rounded text-center block text-xs"
      >
        🟦 Zenzone Direct
      </button>
