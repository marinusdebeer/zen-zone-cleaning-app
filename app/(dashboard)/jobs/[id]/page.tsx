    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="p-4 bg-gray-50 rounded-lg">
            ...
            <Link href={`/clients/${job.client.id}`} className="font-semibold text-gray-900 hover:text-[#4a7c59]">
              {job.client.name} →
            </Link>
          </div>
          {job.property && (
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <p className="font-semibold text-gray-900">{job.property.address}</p>
            </div>
          )}
          {job.estimatedCost && (
            <div className="p-4 bg-gray-50 rounded-lg">
        ...
        {job.convertedFromEstimate && (
          <div className="mt-4 p-4 bg-blue-50 border-l-4 border-blue-500 rounded-lg">
            <p className="text-sm text-blue-800">
              ...
              <Link href={`/estimates/${job.convertedFromEstimate.id}`} className="ml-2 underline hover:text-blue-900">
                View Estimate →
              </Link>
            </p>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        {upcomingVisits.length > 0 && (
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Upcoming Visits</h3>
            <div className="space-y-3">
              {upcomingVisits.map(visit => (
                <div key={visit.id} className="p-4 border-2 border-blue-200 bg-blue-50 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                        ...
                        {visit.isManual && (
                          <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">Manual</span>
                        )}
                      </div>
                    ...
                    <span className={`px-2 py-1 rounded text-xs font-medium ${
                      visit.status === 'InProgress' ? 'bg-yellow-100 text-yellow-800' :
        ...
        {completedVisits.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-gray-700 mb-3">Completed Visits</h3>
            <div className="space-y-3">
              {completedVisits.map(visit => (
                <div key={visit.id} className="p-4 border border-gray-200 rounded-lg bg-green-50">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2">
                    ...
                    <span className="px-2 py-1 bg-green-100 text-green-800 rounded text-xs font-medium">Completed</span>
                  </div>
                </div>
              ))}
            </div>
